...
| REQ-MOD-001 | `/dashboard/curator` is a stable shell that receives a server-issued, allow-listed, versioned module registry and renders workflow modules only through approved shared intake/evidence/map/policy/review/receipt components. | AD-63, canonical 25.3b | Dependency-blocked — local source-controlled registry remediation is pending verification and independent review; it is not accepted implementation evidence |
| REQ-MOD-002 | Workflow modules may define presentation and typed workflow descriptors but cannot load arbitrary client code, query storage, select scope, map identity, authorize, evaluate policy, mutate state, issue receipts, or redefine standard truth states. | AD-63, canonical 25.3b | Dependency-blocked — local source-controlled registry remediation is pending verification and independent review; it is not accepted implementation evidence |
| REQ-MOD-003 | Unknown, duplicate, incompatible, untrusted, capability-missing, disabled, or failed modules fail closed and are independently rollbackable without affecting the shell, other modules, engine, SDK, API, MCP, CLI, or external host adapters. | AD-63, canonical 25.3b, 25.6 | Dependency-blocked — local source-controlled registry remediation is pending verification and independent review; it is not accepted implementation evidence |
