...
- **Status:** Decided
- **Decision:** Governed retrieval resolves relational facts and hard boundaries in PostgreSQL before any semantic/vector candidate expansion. For relational entity families that require semantic discovery, Allura creates deterministic, redaction-aware Markdown `SemanticProjection` documents that assemble the meaningful header/detail relationship before embedding; it never embeds an isolated row as if it represented the whole transaction.
- **Rationale:** A vector index over chunked documents or one relational row cannot replace the operational facts already held in related records. Treating semantic results as primary authority risks omitted detail, cross-scope leakage, stale state, and unsupported conclusions.
- **Consequences:** `memory_search` must evolve from its current vector-first/fallback-trace ordering into a typed retrieval plan. Responses expose relational sources, semantic sources, evidence references, freshness, and degraded state. Semantic search remains useful for discovery, not authority.
- **Owner:** Brooks (architecture) + Troy (implementation); Knuth validates query/isolation behavior.
- **References:** Story 25.2a; REQ-CUR-009; AD-50; AD-57.

### AD-59: Workspace Memory Map Query Contract and Rendering Budgets

- **Status:** Decided
- **Decision:** Allura maps are server-owned, focused subgraphs. Every public surface uses one `SubgraphQuery`/`SubgraphResponse` contract with server-derived tenant/workspace/role/policy context, relational anchor resolution before semantic expansion, evidence-bound nodes/edges, signed continuation, and explicit complete/partial/empty/denied/degraded states. 2D is the required default; 3D is an optional renderer over the same response only.
- **Rationale:** The existing graph route accepts caller-selected scope, has schema/query drift, and eagerly returns a capped graph. A map cannot become a reliable review surface until scope, relation evidence, ordering, and truncation are contracts rather than UI behavior.
- **Consequences:** Initial response caps are 200 nodes, 400 edges, and depth 2; these are safety budgets, not scale claims. Every node/edge needs evidence or a declared versioned derived rule. Continuations bind to normalized query, derived scope, policy version, snapshot cutoff, expiry, and stable sort position. No whole-workspace graph, fake graph, or offset traversal is permitted.
- **Owner:** Brooks (architecture), Knuth (data contract), Pike (2D interaction), Bellard (measurement), Hightower (flag/rollback).
- **References:** 25.2a, 25.2, 25.3, 25.3a, REQ-MAP-001–003.

### AD-60: Governed Assistant Authority and Cross-Surface Parity

- **Status:** Decided
- **Decision:** The first `GovernedAssistant` is a selected-item-scoped, read-only application service. It returns an answer, authorized citations, the same `RetrievalPlan` as map/detail, declarative allowed-action hints, and explicit complete/partial/degraded/denied state. Dashboard, REST API, SDK, MCP, and CLI adapt the same service contract.
- **Rationale:** A normal chatbox would create a second retrieval and authority plane. A cited explanatory service makes the AI dashboard useful without allowing it to approve, write, connect externally, or claim results the governed system has not issued.
- **Consequences:** The assistant cannot choose scope, call raw storage, mutate memory, approve/reject/promote, invoke connectors, mint receipts, override policy, or conceal degraded evidence. The first prompt is `Ask about this item`; insufficient evidence produces `I can’t verify that yet` with inspectable known sources.
- **Owner:** Brooks (authority), Jobs (scope), Woz/Troy (implementation), Bellard (parity harness), Pike (interface).
- **References:** 25.4a, REQ-AST-001–002.

### AD-61: Portable Agent Skills and Microsoft Entra Client Mapping

- **Status:** Decided
- **Decision:** Allura maintains one host-neutral Agent Skill source and thin adapters for Microsoft Copilot Cowork, Claude Code, and Codex. Cowork uses an original Microsoft 365 app package and remote HTTPS MCP connector; Claude Code and Codex use native skill/plugin packaging. Microsoft Entra users, groups, and app roles are validated and mapped server-side to Allura principals, memberships, workspaces, and roles. All authority remains in Allura's scope, policy, evidence, mutation, and receipt services.
- **Rationale:** Portable skills demonstrate reusable framework design without allowing host-specific workflow, identity, or policy forks. Entra provides the requested Microsoft role-based identity source for Cowork while Allura preserves a neutral internal principal/role contract for other hosts.
- **Consequences:** Story 25.4b is blocked on 25.2 and 25.4a. Skills, forms, plugins, and optional widgets confer no permission. Raw claims are never database predicates; unknown/overage/missing/stale/disabled/forged identity conditions fail closed. Exa-backed research remains provisional until a confirmation-required Allura action records it. Each host adapter is independently disableable and never blocks local dashboard, API, SDK, MCP, or CLI operation.
- **Owner:** Brooks (architecture), Troy/Woz (portable adapters), Pike (elicitation/widget UX), Bellard (identity/parity/denial harness), Hightower (Entra/auth/deploy/disable).
- **References:** Story 25.4b; REQ-COP-001–003; AD-57; AD-58; AD-60.

### AD-62: Mortgage Approval Gate Replaces Salesforce-Oriented Vertical Demo

- **Status:** Decided; supersedes the active direction in AD-39 that kept mortgage work paused. Historical Mortagate/Salesforce references remain archive context only.
- **Decision:** The external portfolio demonstration is the vendor-neutral **Mortgage Approval Gate**: `intake → evidence/OCR → policy evaluation → human review → immutable receipt`, exercised through Copilot Cowork, Claude Code, and Codex over the same Allura contracts. Salesforce is not a dependency, integration, schema, CRM, or demo surface.
- **Rationale:** A regulated-style gate demonstrates Allura's actual differentiators—source provenance, OCR uncertainty, deterministic policy, human authority, cross-host parity, denial behavior, and receipts—without spending portfolio value on Salesforce-specific assembly.
- **Consequences:** Story 25.5a depends on 25.4b and 25.5 and becomes part of the 25.6 demo gate. Fixtures are synthetic and contain no applicant PII or customer data. The demo makes no automated underwriting, lending/credit decision, fair-lending, compliance-certification, regulatory-production, or employer/vendor endorsement claim.
- **Owner:** Jobs (scenario scope), Brooks (architecture), Troy/Woz (implementation), Pike (workflow UX), Bellard (parity/adversarial evidence), Hightower (identity/deployment/rollback).
- **References:** Story 25.5a; AD-61; 25.6; Mortgage Approval Gate demo definition in the Epic 25 brief.

### AD-63: Modular Governed Dashboard Shell and Workflow Modules

- **Status:** Decided
- **Decision:** `/dashboard/curator` is a stable governed shell that composes allow-listed, versioned `WorkflowModuleManifest` definitions through shared intake, evidence, map, policy, human-review, and receipt components. Mortgage Approval Gate is the first module and is not hardcoded into dashboard routes or authority services.
- **Rationale:** Domain workflows should demonstrate Allura's reusable framework without turning the product into a mortgage-specific application or creating separate route/service stacks for every vertical. A declarative module boundary preserves conceptual integrity and makes the same workflow portable to Cowork, Claude Code, and Codex.
...
- **Operational Agent Access**: governed MCP/API/CLI interfaces backed by PostgreSQL; browser surfaces are optional adapters.
- **Durable Store**: PostgreSQL holds append-only events, canonical proposals, pgvector retrieval, graph tables, receipts, and outbox state. Neo4j is sunset under AD-50; active references are migration debt tracked by Epic 25.1.
