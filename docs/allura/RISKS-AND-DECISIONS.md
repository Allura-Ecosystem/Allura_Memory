...
| AD-02 | PostgreSQL (graph_memories) for semantic memory                                         | Superseded by AD-44 | Native graph relationships, SUPERSEDES lineage is idiomatic, semantic search via Cypher. **Sunset 2026-07 — PostgreSQL (graph_memories) container stopped. All semantic storage now PostgreSQL pgvector (see AD-44).**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| AD-04 | `PROMOTION_MODE` env var governs PostgreSQL (graph_memories) writes                     | Decided  | Architectural gate. `soc2` mode requires human approval — enforced in code, not policy. `auto` mode uses score threshold.                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| AD-05 | 5-tool public API surface (`add/search/get/list/delete`)          | Decided  | mem0 UX parity. Developers see five tools. All internal complexity is hidden. `memory_add` authority is transport-bound: authenticated MCP/stdio code alone derives tenant/workspace/actor/session and mints the non-serializable write capability. REST `POST /api/memory` is fail-closed until it has the same verified workspace-principal boundary. |
...
| AD-09 | PostgreSQL (graph_memories) write failure is non-fatal                                  | Superseded by AD-44 | Postgres write is the source of truth. PostgreSQL (graph_memories) was a promotion layer — its failure should degrade gracefully. **Neo4j sunset 2026-07 — this decision is now moot. All storage is PostgreSQL.** |                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| AD-14 | PostgreSQL required, PostgreSQL (graph_memories) degradable at runtime                  | Superseded by AD-44 | Canonical memory contract remains available when PostgreSQL is healthy. PostgreSQL (graph_memories) was optional-at-runtime. **Neo4j sunset 2026-07 — PostgreSQL is now the sole storage engine. All graph/semantic operations use pgvector.** |                                                                                                                                                                                                                                                                                                                              |
...
| AD-19 | Controlled retrieval layer as sole agent read path                | Decided  | Agents MUST NOT query PostgreSQL directly. All reads go through `POST /api/memory/retrieval`. This enforces scoping, audit logging, and policy at the service boundary rather than relying on agent compliance. See [BLUEPRINT.md](./BLUEPRINT.md).                                                                                                                                                                                                                                                                          |
...
| AD-26 | Real-data API uses query/mapper boundary and read-only graph endpoint | Decided | Issue #25 makes `/api/memory` a trust surface over real Allura Brain data, not a mock endpoint. API consumers use mapped contracts from `src/lib/api/`; raw Brain API shapes stay behind `api.ts`, `queries.ts`, and `mappers.ts`. `/api/memory/graph` is read-only, tenant-scoped by `group_id`, returns a capped display sample plus `total_edges`, and performs no PostgreSQL (graph_memories) mutations. Alternatives rejected: mock data, client-inferred relationships, and raw API responses in consumers. |
...
| AD-34 | RVF MCP and Cognitum Gate — Level 4 self-evolution approved | Accepted | ~~Deferred~~ — **Unparked 2026-07-17 by Captain.** SONA feedback loops, coherence monitor, genesis engine, self-healing, and pattern learning are approved for native build in allura-memory repo. HITL gates remain on all self-modification. See epic: `_bmad_output/planning-artifacts/epic-level-4-pattern-learning.md`. | |
...
| AD-40 | Control Plane as single write gate for all DB operations | Decided | syscall_mutate and syscall_query are now the only path to PostgreSQL and PostgreSQL (graph_memories). agentMemory (MCP) and memory() (internal graph) both route through the control plane's proof→policy→audit pipeline. Direct DB backends available via MEMORY_BYPASS_CONTROL_PLANE=true for migration (renamed 2026-08-20 from MEMORY_BYPASS_KERNEL, which is still read as a deprecated fallback with a warning). Target resolver enforces append-only on pg:events, group_id on all ops, label allowlist on PostgreSQL (graph_memories). Satisfies F1, F8, F26, F27, F33, F35, F37, F38. |
...
| AD-49 | RuVector Graph Cutover — default to `GRAPH_BACKEND=ruvector` (PG tables) | Decided | Removes per-person graph-auth wall (PostgreSQL (graph_memories) Community = 1 user), collapses two stores toward one engine, self-hosted, no license tier. Two paths: Path A (PG tables, ship now — recommended for beta), Path B (ruvnet Rust crate, upstreamable — Sabir's choice, spike passed 2026-06-24). Naming: Allura's `ruvector` backend is a PG-table implementation named after the concept, NOT a binding to the ruvnet Rust crate. Story 19.3 executed the flip (2026-07-12); PostgreSQL (graph_memories) remains as fallback for one release. Related: AD-29 (graph adapter pattern), AD-34 (defers full RuVector-Postgres migration — this activates it). |
| AD-50 | PostgreSQL (graph_memories) sunset — PostgreSQL-only architecture | Decided | 2026-07-17: Captain confirmed PostgreSQL (graph_memories) is sunset. All memory operations (episodic + semantic) are now PostgreSQL + pgvector only. PostgreSQL (graph_memories) container is stopped. This supersedes AD-02 (PostgreSQL (graph_memories) for semantic memory), AD-09 (PostgreSQL (graph_memories) write failure non-fatal), and AD-14 (PostgreSQL required, PostgreSQL (graph_memories) degradable). AD-49 RuVector cutover (2026-07-12) was the operational transition; this AD formalizes the sunset. All graph operations, SUPERSEDES lineage, and semantic search now use pgvector tables in PostgreSQL. No code should write to or read from PostgreSQL (graph_memories). |
...
| AD-57 | Bumblebee V1 alert-and-proposal authority boundary | Decided | Bumblebee may automatically ingest allowlisted evidence, correlate verified exposure, create deduplicated alerts, and prepare simulated mitigation proposals. It may not activate policy, block CI/packages, change schedules, or perform containment. This preserves rapid detection while retaining human authorization and receipts for enforcement. |
| AD-58 | "Security owner" authority resolves to the existing `admin` RBAC role | Decided | 2026-08-27: Stories 26.1, 26.4, and 26.6 all name a "security owner" who approves scheduler config, source allowlists, and (26.6) containment authorization -- a concept that does not exist in `src/lib/auth/types.ts`'s three-role hierarchy (`viewer < curator < admin`). Rather than introduce a fourth role for a distinction with no current second admin-tier principal to separate it from, "security owner" is declared to mean the existing `admin` role for all Bumblebee (Epic 26) approval gates. No RBAC schema, Clerk mapping, or `hasPermission()` change required. Alternative considered: a distinct `security_owner` role -- rejected for now as unwarranted complexity (touches Clerk role wiring, every DB CHECK constraint enumerating roles, and every `hasPermission()` call site) for a distinction the system has no present need to enforce; revisit if/when a second admin-tier principal needs to be excluded from security-owner authority specifically. Resolves Story 26.6's AC-6 dependency ("role-model reconciliation is complete before response authorization is exposed"). |
| AD-59 | `endpoint_isolation` containment is descoped: Allura owns no endpoint to isolate | Decided | 2026-08-27: Story 26.6's AC-3 names three containment targets -- token revocation, workspace locks, and "endpoint actions". The first two have real targets in this system (`mcp_tokens.revoked_at`, `workspaces.lock_mode`) and are implemented and gated. The third does not: a repository-wide search for a host/device/network endpoint concept found only HTTP API routes (`src/lib/auth/route-scope-manifest.ts`) and third-party advisory API URLs (`src/lib/threat-ingestion/*`), neither of which can be "isolated" in the security sense the AC intends. This is a boundary fact, not a backlog item -- Allura is a memory and governance control plane; it runs no agent on a host, manages no devices, and sits in no network path. Decision: `endpoint_isolation` remains in the connector type union and **fails closed** -- `executeContainmentAction` throws before any receipt is written or any state is touched -- rather than being deleted (which would lose the explicit refusal) or implemented (which would require inventing an EDR/MDM/network integration that does not exist, producing a connector that reports success having done nothing). Alternatives considered: (a) delete the enum member -- rejected, the explicit fail-closed refusal is itself the safety property worth keeping and tested; (b) build a stub that returns success -- rejected outright as untestable theatre in a security-critical path. Revisit if Allura ever gains a real endpoint (e.g. an agent running on a managed host). Pinned by the test "rejects endpoint_isolation regardless of role or approval" in `src/lib/containment/__tests__/governed-authorization.test.ts`. |
| AD-60 | Upstream Bumblebee integrates as a headless Allura plugin with server-issued scan leases | Decided | 2026-08-27 Correct Course accepted after Pike/Fowler/Knuth remediation and final staged-diff PASS. Pin upstream `perplexityai/bumblebee` `v0.1.2` at `cc57710eeaf685e7b89924a36c8583cad0a378fe`. The read-only scanner remains upstream. A long-lived `bumblebee_runner` credential may request a source/population/catalog-bound monotonic lease only; the resulting short-lived `bumblebee_ingest` credential may post NDJSON only. Neither audience authenticates to the other route, MCP, or browser. Allura derives tenant/workspace from verified server authority, stores sanitized immutable evidence, and promotes only a valid complete bound population. Findings-enabled leases reference an immutable scoped catalog revision/entry ledger. The generic Curator registry is optional display infrastructure, not scanner transport or an Epic 26 dependency. |
...
|----|-------|----------|--------|
| RK-01 | PostgreSQL (graph_memories) graph bloat from duplicate promotions | Medium | Active |
...
| RK-07 | Schema drift between Postgres and PostgreSQL (graph_memories) | High | Active |
| RK-08 | Embedding provider latency spikes | Medium | Active |
| RK-09 | PostgreSQL (graph_memories) promotion conflicts (concurrent writes) | High | Mitigated |
...
| RK-32 | RuVector Graph Cutover Risk | Medium | ✅ Resolved — Story 19.3 flip executed (2026-07-12): `GRAPH_BACKEND=ruvector` default, live-DB E2E passed (14/14), dual-read validated, parity test 14/14 green. PostgreSQL (graph_memories) remains as fallback for one release as per AD-49. 5 sub-risks: R1 Cypher-subset (✅ Resolved for Path A), R2 SUPERSEDES immutability (✅ Resolved), R3 Maturity/breaking changes (✅ N/A Path A), R4 Fulltext+constraints (✅ Resolved Path A), R5 Live-DB E2E proof (✅ Resolved — 2026-07-12). |
...
| RK-35 | Serializable memory-write scope could be replayed or forged | High | Mitigated — raw `memory_add` rejects plain scope and only authenticated transport-local capability code can bind a request. Regression coverage rejects forged principals plus JSON and object-spread copies of a prepared request. |

### Risk Detail

| ID | Risk | Impact | Mitigation | Status |
| ---- | ------------- | ------ | --------- | -------- |
| RK-01 | PostgreSQL (graph_memories) graph bloat from duplicate promotions   | Medium | Dedup check before every PostgreSQL (graph_memories) write (`src/lib/dedup/`)                                                                | Active    |
| RK-02 | Cross-tenant data leakage                     | High   | `group_id` CHECK constraint + scoped queries on every Postgres + PostgreSQL (graph_memories) operation                                       | Mitigated |
...
| RK-05 | Packaged MCP server coordination failures     | Medium | Use focused packaged MCP servers with memory-first routing: `neo4j-memory` first, `database-server` second, `neo4j-cypher` only when needed. Failure of one inspection server should degrade to remaining available surfaces; Postgres + PostgreSQL (graph_memories) remain the durable stores. | Accepted  |
...
| RK-07 | Schema drift between Postgres and PostgreSQL (graph_memories)       | High   | Sync validation in health check endpoint. Schema version field in both stores. Alert on mismatch.                      | Active    |
...
| RK-09 | PostgreSQL (graph_memories) promotion conflicts (concurrent writes) | High   | Optimistic locking via `version` field. Retry on `SUPERSEDES` conflict. Max 3 retries.                                 | Mitigated |
...
| RK-12 | Retrieval layer bypass — agents query DBs directly | High | AD-19: Controlled retrieval layer enforced as sole read path. Code review gate checks for direct PG/PostgreSQL (graph_memories) imports in agent-facing code. Control Plane `direct-access-blocker.ts` detects violations. | Active |
...
- Graph: real PostgreSQL (graph_memories) data only, source receipts, fallback list when degraded.
...
- PostgreSQL (graph_memories) owns semantic relationships and approved projections, not board state.
...
│   PostgreSQL (episodic + semantic) + embeddings                   │
└─────────────────────────────────────────────────────┘
```
...
- **Rationale**:
  - **Removes the per-person graph-auth wall.** PostgreSQL (graph_memories) Community supports exactly one user (`PostgreSQL (graph_memories)`); per-partner graph logins require paid Enterprise. PG tables have no separate login surface — access is governed by *our* control plane + MCP tokens.
  - **Collapses two stores toward one engine.** Graph + vector both live in Postgres; removes the PostgreSQL (graph_memories) + cross-store consistency burden.
...
- **Consequences**:
  - The `ruvector-adapter.ts` / `ruvector-adapter.ts` seam is the migration boundary; nothing else knows which engine backs it.
  ...
  - PostgreSQL (graph_memories) stays as read-only fallback for one release after the flip.
...
| --------------------------- | ---------------------------- | --------------------- |
| PostgreSQL (graph_memories) promotion failures    | `src/lib/PostgreSQL (graph_memories)/promotion.ts` | > 5 failures in 5 min |
...
- **Durable Stores**: PostgreSQL (events/traces) + PostgreSQL (graph_memories) (semantic graph) accessed through controlled services
