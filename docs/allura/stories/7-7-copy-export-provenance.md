...
source_docs:
  - docs/allura/EPICS-dashboard-v2.md
  - docs/allura/PRD-dashboard-v2.md
  - docs/allura/ARCHITECTURE-dashboard-v2.md
  - docs/allura/DATA-SUPPORT-dashboard-v2.md
  - docs/allura/UX-dashboard-v2.md
  - docs/allura/CONTRACTS-dashboard-v2.md
  - docs/allura/KANBAN-dashboard-v2.md
pipeline_status:
  validation: FAIL
...
For this story-spec task, verification is: file exists at `docs/allura/stories/7-7-copy-export-provenance.md`, source anchors are present, no product code changed, and the Kanban task records the intervention/evidence.
...
- PRD-dashboard-v2.md: FR36–FR42 (memory/provenance/audit/export requirements), NFR27 (MIT baseline includes memory provenance and audit foundations).
- ARCHITECTURE-dashboard-v2.md: `AuditEvent` interface frozen (id, group_id, actor_id, actor_type, resource, action, before, after, evidence_ids, policy_decision_id, approval_decision_id, timestamp, hash, prev_hash); `GET /api/audit/events` supports pagination/filter/export.
- DATA-SUPPORT-dashboard-v2.md: Audit is PostgreSQL append-only with chained hash fields; no direct semantic writes.
- UX-dashboard-v2.md: Flow 4 — Human Memory Recall includes "provenance copy/export" as a metadata facet.
- CONTRACTS-dashboard-v2.md: `AuditEvent` contract freeze gate; export schemas must be strict supersets.
- KANBAN-dashboard-v2.md: Epic 7 assigned to Team RAM; scope includes hybrid search + audit chain.
