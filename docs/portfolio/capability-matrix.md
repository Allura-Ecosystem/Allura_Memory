...
| Episodic and semantic persistence use PostgreSQL/RuVector | `implemented` | `measured` | `src/lib/postgres/`, `src/lib/graph-adapter/`, `docker/postgres-init/` | `bash scripts/ci/run-live-db-tests.sh` | Live-DB lane run `32369489097` (`10a6324b`); AC matrix: 37 tenant-scoped tables with forced RLS, `events` UPDATE/DELETE rejected for the app role |
| Promotion is review-gated and auditable | `implemented` | `validated` | `src/curator/`, `src/control-plane/`, `src/mcp/governance-tools.ts` | `bun run test:unit` | Unit test report; remote CI artifact pending |
| Benchmark harness executes black-box MCP scenarios | `implemented` | `measured` | `src/__benchmarks__/` | `bun run benchmark -- --ci-baseline --require-gateway` | Benchmark lane in run `31945804098` produced exact 5-ID inventory (`retrieval-quality`, `curation-accuracy`, `governance-integrity`, `latency-profile`, `audit-completeness`) |
