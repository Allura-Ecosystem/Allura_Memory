...
| F2 | `POST /workspaces` creates a workspace and generates `group_id` (`allura-<slug>`), validated against `^allura-[a-z0-9-]+$`. |
...
|--------|------|------|----------|--------|
| POST | `/orgs` | `{name}` | `{id, name}` | 401, 409 |
| POST | `/workspaces` | `{org_id, name}` | `{id, group_id, lock_mode}` | 401, 403, 409 |
...
- `group_id` is generated server-side and immutable after creation (AD-01).
...
- **AUTH-UC1:** Owner creates org → creates workspace → receives `group_id`.
