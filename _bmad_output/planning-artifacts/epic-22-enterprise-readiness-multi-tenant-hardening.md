...
> Current delivery status is controlled by [`sprint-status.yaml`](../implementation-artifacts/sprint-status.yaml).
...
- **22.5** Build profile export/import — document the workflow: `docker mcp profile export faithmeats ./faithmeats-profile.yaml` → commit to repo → new machine does `docker mcp profile import ./faithmeats-profile.yaml`. Add a README in `_bmad_output/planning-artifacts/profiles/` documenting each profile, its tenant, and its tool restrictions.
