# Retrospective — Epic 23: PostgreSQL (graph_memories) Sunset Completion

**Date:** 2026-07-29
**Epic:** 23 — PostgreSQL (graph_memories) Sunset Completion — Clean Codebase
...
| Dead PostgreSQL (graph_memories) files | ~20 | 0 (deleted) |
| Files changed | — | 91 |
| Lines deleted | — | ~10,328 |
| Lines added | — | ~1,651 |
| Token compliance | 19 hex + 13 deprecated | 0 + 0 |
| Commits | — | 3 |
| Source files with PostgreSQL (graph_memories) refs | 90+ | ~20 (mostly comments + defensive checks) |
...
2. **The exit gate for "0 PostgreSQL (graph_memories) references" was unrealistic.** 843 references remain — mostly comments, defensive checks, and the `neo4j-driver` type import. The gate should have said "0 active PostgreSQL (graph_memories) runtime code paths" instead of "0 grep results." The gate was too strict and the code review correctly flagged this as a partial pass.
...
4. **No transaction wrapping in promoteToPostgreSQL (graph_memories)().** The rewrite of knowledge-promotion.ts did two INSERTs without a transaction. The code review caught this (W1), but it should have been in the original implementation. When rewriting code that does multiple dependent writes, transaction wrapping is not optional.

## Key Insights

1. **PostgreSQL (graph_memories) sunset was a 3-epic process, not one.** Epic 19 flipped the default. Epic 22 removed the container. Epic 23 cleaned the code. Each was necessary. Skipping the cleanup phase leaves a codebase that looks done but has 25 failing tests and broken typecheck.
...
3. **Code review gates prevent real security regressions.** The `executeCypher()` raw SQL issue was introduced by the PostgreSQL (graph_memories)→PostgreSQL rewrite. Without the adversarial review, it would have shipped to production.
...
1. **W4: health/metrics/route.ts** still checks PostgreSQL (graph_memories) health — remove the block
2. **W5: startup-validator.ts** still has PostgreSQL (graph_memories) check functions — remove them
3. **W6: scripts using throwing stubs** — rewrite or deprecate
4. **S1: Rename promoteToPostgreSQL (graph_memories)() → promoteToKnowledgeGraph()** — misleading name
5. **Clean up remaining 843 PostgreSQL (graph_memories) references** — mostly comments, but tidy up
...
| Dead code removed | ✅ src/lib/PostgreSQL (graph_memories)/ deleted |
| Code review | ✅ C1 fixed, W1-W3 fixed, W4-W7 tracked |
| Pushed to GitHub | ✅ commit e9e0eb22 on main |

## Next Steps

1. Track W4-W7 as follow-up stories (PostgreSQL (graph_memories) Sunset Phase 2)
