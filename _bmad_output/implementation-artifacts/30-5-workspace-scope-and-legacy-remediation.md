...
2026-09-25: the protected Epic 30 memory-route inventory now forbids raw caught exceptions from being passed to console logging or exception telemetry. Runtime sentinel tests prove generic error responses and static logging for a backend failure while preserving an authorized read; a lexical inventory covers root, item, aggregate, trace, graph, restore, history and bulk-erasure routes. This repair was prompted by the controlled-red test exposing raw backend sentinel text in server logs. The exact Epic 30 gate passes typecheck with 293 tests and 3 intentional skips across 31 files; the full unit lane passes 2,855 tests with 165 skips across 191 files. This is local hermetic log/telemetry containment, not external observability audit, live database proof, human review or Story 30.5 acceptance.
