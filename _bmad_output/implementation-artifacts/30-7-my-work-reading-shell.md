...
2026-09-25: commit `ac3e70a9` closes the remaining local memory-tab regression gap. The component test now proves duplicate opens retain exactly two unique tabs, Home/End and both arrow directions move selection and focus correctly, and `tabIndex` roves so exactly one tab remains in the sequential focus order. The focused component lane passes 30/30 tests and the exact hermetic lane passes 364 tests with 3 intentional skips across 37 files. This is automated keyboard evidence only; the post-tab design comparison, 320px/200% browser proof, screen-reader journey, human accessibility approval and production-provider activation remain open.

2026-09-25: commit `104bf29a` adds a fail-closed production-provider state mapper without activating a provider. It validates server-owned tenant/workspace/principal scope, exact document shape, workspace binding and visibility consistency; strips documents from every non-content state; and converts malformed or unknown provider output to content-free `unavailable`. The ordinary dashboard remains unchanged and synthetic confinement remains explicit. Eight new read-service tests plus page tests are pinned in the exact lane. Production provider authority/activation, live proof, approved design acceptance, screen-reader and 200% zoom evidence remain open. The current exact lane passes typecheck with 355 tests and 3 intentional skips across 37 files; the full unit lane passes 2,917 tests with 165 skips across 197 files.
