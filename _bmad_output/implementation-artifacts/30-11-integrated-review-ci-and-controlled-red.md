...
2026-09-25 hosted exact-SHA evidence: authenticated GitHub Actions run [36170672360](https://github.com/Allura-Ecosystem/Allura_Memory/actions/runs/36170672360) completed successfully on exact local-main SHA `7ef1464da6e6360f8797769b58e9d92b7ea8610d`. Job `108189153342` passed the Epic 30 hermetic authorization/UI lane; job `108189153617` passed the confined PostgreSQL/HTTP lane and stopped its service container; job `108189153618` passed the repository Check lane (lint, unit and integration). The first hosted attempt exposed a 2-second readiness-probe cold-start defect; the bounded repair retains the 60-second overall ownership deadline while allowing a single Next.js cold compilation up to 15 seconds, and its focused owned-process lane passes 11/11 tests. No production database, protected-main mutation or branch-protection bypass occurred. Exact machine-readable evidence is in [the hosted run receipt](./evidence/epic30-hosted-run-36170672360.json). This closes `hosted-exact-sha-ci` only. Required protected-branch check configuration and independent human reviews remain open, so Story 30.11 remains partial/backlog.
