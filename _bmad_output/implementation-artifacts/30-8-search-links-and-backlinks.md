...
2026-09-25: commits `3437da4f` and `d96302bb` add an unactivated receipt-gated production relationship/search candidate over the restricted workspace transaction. Search treats `%`, `_` and backslash literally; pages are bound to an authenticated prior-receipt witness and reauthorized after receipt acknowledgement; links/backlinks disclose only endpoints present in the current authorized snapshot; citations omit absent endpoints; and derivatives fail closed unless every source remains present. One shared canonical-ID validator now rejects whitespace aliases, controls, pipe aliases and markup across synthetic and production derived reads. No route or concrete writer is activated. Independent review found no remaining BLOCK/HIGH issue; the exact hermetic lane passes 364 tests with 3 intentional skips across 37 files. Timing analysis, replay disposition, live revocation proof, approved relationship policy, production activation and human review remain open, so Story 30.8 remains backlog.

2026-09-25: the synthetic My Work shell now enables its searchbox as a client-only filter over the already server-authorized `documents` snapshot. Matching is case-insensitive over permitted titles and content, updates only the private/department navigation lists, preserves the active authorized reading tab, makes no fetch or Brain/provider call, and clears with Escape or the native search control. A query with no permitted match returns only `No authorized matches.` without a name, count, snippet, facet, identifier or hidden-existence distinction. The focused component lane passes 30/30 tests, lint and typecheck pass, independent review found no BLOCK/HIGH/MEDIUM issue, the exact Epic 30 lane passes 329 tests with 3 intentional skips across 34 files, and the full unit lane passes 2,891 tests with 165 skips across 194 files. This closes the disabled local search-affordance gap only. Production search remains quarantined; approved search authority, production provider mapping, timing/live revocation proof, browser/human accessibility evidence and Story 30.8 acceptance remain open.

2026-09-25: the one-hop synthetic relationship candidate now validates canonical focus and reference IDs without trimming aliases into another document ID. It rejects empty/whitespace/overlong/control/markup/pipe-alias focus IDs before reading and ignores non-canonical references such as `[[ target ]]`. Successive-call tests prove focus, target and backlink relationships disappear as the shared authorized snapshot is revoked; no relationship cache or fallback is used, and serialized output contains only endpoint IDs/titles. The exact Epic 30 gate passes typecheck with 257 tests and 3 intentional skips across 28 files; the full unit lane passes 2,823 tests with 165 skips across 189 files. Independent AI review found no remaining BLOCK/HIGH/MEDIUM issue after the canonical-reference repair. This remains synthetic-only evidence, not approved relationship policy, timing proof, live PostgreSQL/revocation proof, independent human review or story acceptance.
