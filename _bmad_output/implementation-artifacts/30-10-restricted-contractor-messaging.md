...
2026-09-25: commit `0ed4cdeb` makes the durable production candidate conform to the provider-neutral `MessagingStore` contract while remaining read-only and unactivated. The store is constructor-bound to one server-derived tenant/workspace/principal/session/role/policy-epoch scope, rejects bound-scope mismatches before SQL, rechecks current transaction-local tenant/workspace/principal plus active provenance-bearing membership and epoch, and limits contacts, approvals, invitations and message read-back to the exact bound scope. Invitation queries now require both trusted approval sources with non-empty provenance and the current epoch. Atomic invitation/message commits still return `false`; no mutation, provider delivery, route or UI exists. Independent review found no BLOCK/HIGH/MEDIUM issue. The exact no-cache Epic lane passes 375 tests with 3 intentional skips across 38 files; the full unit lane passes 2,937 tests with 165 skips across 198 files. A concrete restricted transaction wrapper, governed writers, delivery provider, UI/accessibility, live RLS/read-back/revocation timing, human review and release evidence remain open.
