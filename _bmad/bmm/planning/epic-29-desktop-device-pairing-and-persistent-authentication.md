...
- **AR10:** Grace window is recovery-only (24h default, configurable `ALLURA_DEVICE_KEY_GRACE_HOURS` 1–72): no normal MCP token or recovery bearer token; returns only a public-safe re-issue of the server-HMAC-authenticated activation receipt after old-key proof; rate-limited per device via `grace_exchange_count` under row lock (architecture §6.5, HIGH-F2).
