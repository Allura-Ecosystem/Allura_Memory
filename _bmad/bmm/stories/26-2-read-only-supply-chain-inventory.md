...
## Allura-local inventory adjunct — 2026-08-27 (historically called “Bumblebee Guard”)
...
worker had zero real packages to poll advisories against. This local parser was
historically called “Bumblebee Guard.” The Epic 26 Correct Course now reserves
**Bumblebee plugin** for the Allura integration around the pinned upstream
`perplexityai/bumblebee` scanner. This local parser remains an adjunct source.
...
- Allura-local inventory adjunct: `src/lib/inventory/__tests__/lockfile-parser.test.ts` (9 tests, including two against this repo's real `bun.lock`), `src/lib/inventory/__tests__/reconciliation.test.ts` (6 tests, including the empty-result safety guard).
...
- agent: Brooks (documentation/verification pass; original implementation by Woz + Knuth, PR #108; local inventory adjunct by Brooks, 2026-08-27)
- date: 2026-08-27
...
- remaining gaps: AC-1's original 6 acceptance criteria remain fully met (unchanged from the earlier verification pass). The local adjunct covers only lockfile/bun.lock. It does not implement upstream Bumblebee transport, snapshot state, or endpoint onboarding; that is replanned in Story 26.7.
