...
- [ ] **Headless end-to-end proof:** on one frozen candidate, a real leased scan reaches HTTPS ingestion, sanitized ledgers, complete-population promotion, current inventory, and exposure retrieval. Evidence covers duplicate, empty, partial/error/missing-summary, filtered/changed population, stale/future clock, malformed/gzip limit, DB failure, conflicting replay, and cross-tenant cases. — **Partially covered — NOT complete.** 2026-08-29 AC-19 review (Pike/Fowler/Knuth) found the earlier "9/9 complete" claim false: only accept, replay, empty, conflict, malformed, oversized, content-type, auth, and cross-tenant RLS are live-proven. Genuinely uncovered: partial/error/missing-summary statuses, filtered/changed population, stale/future clock, gzip/compression limit, and DB-failure paths. Reopen on a new frozen candidate.
...
## Open items (AC-19 review, 2026-08-29 — CHANGES-REQUESTED)

1. **Root defect — spec level (contract item 7), needs Correct Course, not a patch:** `bumblebee_records` PK is `(group_id, workspace_id, source_id, run_id, record_id)` — it omits `source_revision_id`, `lease_id`, `batch_id`, and `run_id` is scanner-supplied. The `bumblebee_run_decisions → bumblebee_records` FK (`summary_record_id`) therefore constrains a looser grain than the decision's own PK: a promoted decision can cite a summary from a different lease, generation, or source revision. **Hold contract item 9 (evidence junctions)** until this is resolved — junctions reference `records`, and building on the current key deepens the inversion.
2. **AC-3 unreachable `authMethod` check** — audit metadata path never exercised in production.
3. **Branch hygiene** — `claude/ac19-remediation` merged; confirm no other divergent branches before the next freeze.
