...
-- Epic 30 governed membership transition.  Approval creation remains outside
-- this boundary: a trusted control plane must insert a verified approval and
-- a separate receipt writer must durably acknowledge the candidate decision
-- before this function can consume either record.  allura_app receives no
-- direct DML grant on the provenance or membership tables.
ALTER TABLE brain_membership_approvals
  ADD COLUMN IF NOT EXISTS consumed_at TIMESTAMPTZ;

CREATE INDEX IF NOT EXISTS brain_membership_approvals_pending_idx
  ON brain_membership_approvals (group_id, workspace_id, approval_id)
...
BEGIN
  ...
  -- The caller must be an active tenant administrator with an active,
  -- provenance-bearing workspace membership at the exact policy epoch.  No
  -- caller supplied role, scope, or epoch is trusted over these rows.
  ...
  SELECT * INTO v_approval
    FROM public.brain_membership_approvals
  ...
  SELECT * INTO v_membership
    FROM public.brain_workspace_memberships
   WHERE group_id = v_group_id
     AND workspace_id = v_workspace_id
     AND user_id = v_approval.subject_user_id
   FOR UPDATE;
  ...
  ELSE
    ...
    END IF;
    UPDATE public.brain_workspace_memberships
...
COMMENT ON FUNCTION app.commit_brain_workspace_membership(UUID,TEXT,BIGINT,BIGINT,BIGINT) IS
  'Epic 30 restricted transition: consumes one verified membership approval only after an exact durable receipt and current server-scoped admin authority check.';

INSERT INTO schema_versions (version, applied_at, description)
VALUES ('075', now(), 'Epic 30 governed workspace membership transition writer')
