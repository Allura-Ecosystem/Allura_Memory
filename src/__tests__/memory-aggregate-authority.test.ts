...
function request(path: string): NextRequest {
  return new NextRequest(`http://localhost:3100${path}`, { headers: {
    "x-allura-user-id": "owner-user", "x-allura-role": "viewer",
...
describe("memory aggregate authority", () => {
  it.each([["count", count], ["stats", stats]])("denies forged scope before %s query", async (_name, handler) => {
  ...
  it("counts only through the verified restricted workspace transaction", async () => {
    mocks.query.mockResolvedValueOnce({ rows: [{ id: "a" }, { id: "b" }] })
    ...
    expect(mocks.transaction).toHaveBeenCalledWith({ tenantId: "allura-system",
      workspaceId: "workspace-a", principalId: "owner-user" }, expect.any(Function))
  ...
  it("computes stats only from workspace-filtered rows", async () => {
    mocks.query.mockResolvedValueOnce({ rows: [{ episodic_count: "2", search_count: "1",
      last_activity: new Date("2026-09-24T00:00:00Z") }] })
    ...
    expect(await response.json()).toEqual({ episodic_count: 2, semantic_count: 1,
      search_count: 1, total_count: 2, last_activity: "2026-09-24T00:00:00.000Z" })
