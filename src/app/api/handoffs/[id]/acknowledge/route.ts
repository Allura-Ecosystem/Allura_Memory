/**
 * Work Plane — Handoff Acknowledge API
 *
 * POST /api/handoffs/:id/acknowledge — acknowledge a pending handoff. Role: curator.
 *
...
import { acknowledgeHandoff, getHandoff } from "@/lib/work-plane/handoff-manager"
...
export async function POST(
  ...
  try {
    ...
    // Check existence before attempting acknowledgement to give accurate 404 vs 409
    const existing = await getHandoff(pool, { id, groupId: validatedGroupId })
    if (existing === null) {
      return NextResponse.json({ error: "Handoff not found" }, { status: 404 })
    }

    const handoff = await acknowledgeHandoff(pool, { id, groupId: validatedGroupId })

    if (handoff === null) {
      // acknowledgeHandoff only operates on status='pending' — the handoff exists
    ...
    captureException(error, {
      tags: { route: "/api/handoffs/[id]/acknowledge", method: "POST" },
    })
    console.error("Failed to acknowledge handoff:", error)
    return NextResponse.json({ error: "Failed to acknowledge handoff" }, { status: 500 })
  }
}
