...
import { REVOCATION_ERROR_STATUS, RevocationError, revokeDevice } from "@/lib/device-pairing/revocation-service"
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    return NextResponse.json(await revokeDevice(getAppPool(), { device_id: parsed.data.device_id, authUser }))
  } catch (error) {
    ...
    console.error("[device-pairing/revoke] error:", error)
    return NextResponse.json({ error: "INTERNAL_ERROR", message: "Unable to revoke device" }, { status: 500 })
  }
}
