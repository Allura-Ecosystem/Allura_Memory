...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  catch { return NextResponse.json({ error: "INVALID_REQUEST", message: "Invalid recovery request" }, { status: 400 }); }
  ...
  try {
    ...
    const result = await recoverViaGrace(getAppPool(), {
    ...
    if (error instanceof RotationError) {
      const status = error.code === "DEVICE_NOT_APPROVED" || error.code === "KEY_EXPIRED" ? 403
        : error.code === "GRACE_LIMIT_EXCEEDED" ? 429 : 401;
