...
function memoryDb() {
  ...
  return { query, registry, proposals, transitions, sqlLog }
}
...
describe("Munari/Rand review gate — only an approved diff becomes a proposal", () => {
  async function reviewedSession(db: ReturnType<typeof memoryDb>) {
    ...
    return runLaneWork(session, { diff: DIFF, evidence_refs: EVIDENCE }, "woz")
  }
  ...
  it("routes an approved diff through the promotion adapter into a curator proposal", async () => {
    ...
    const result = await reviewLaneEvidence(
      session,
      { verdict: "approved", reviewer: "munari-rand", reason: "concept evidence complete" },
      db as never,
    )
...
describe("no duplicate workflow-status ledger", () => {
  ...
  it("writes only branch_registry, promotion_proposals, and approval_transitions", async () => {
    const db = memoryDb()
    const lane = TEAM_RAM_LANES.find((candidate) => candidate.id === "agent-lane-woz")
    if (!lane) throw new Error("agent-lane-woz missing from lane config")
    const session = await openLane(
    ...
    const worked = await runLaneWork(session, { diff: DIFF, evidence_refs: EVIDENCE }, "woz")
    await reviewLaneEvidence(worked, { verdict: "approved", reviewer: "munari-rand", reason: "ok" }, db as never)
    ...
    expect([...tables].sort()).toEqual(["approval_transitions", "branch_registry", "promotion_proposals"])
  })
