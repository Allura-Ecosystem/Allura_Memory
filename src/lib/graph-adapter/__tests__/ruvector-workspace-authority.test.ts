...
describe("RuVectorGraphAdapter workspace authority", () => {
  it.each(["getMemory", "searchMemories", "listMemories"] as const)(
    "workspace-filters %s and excludes quarantine",
    async (method) => {
      ...
      if (method === "getMemory")
        await adapter.getMemory({ ...base, id: "00000000-0000-4000-8000-000000000001" as never })
  ...
  it("creates memories through the app-role workspace transaction", async () => {
    const client = { query: vi.fn(async (_sql: string, _values?: unknown[]) => ({ rows: [], rowCount: 1 })) }
    transactionState.client = client
  ...
  it.each(["softDeleteMemory", "restoreMemory"] as const)("runs %s inside the exact workspace", async (method) => {
    const client = { query: vi.fn(async (_sql: string, _values?: unknown[]) => ({ rows: [], rowCount: 1 })) }
    transactionState.client = client
    const adapter = new RuVectorGraphAdapter({ query: vi.fn(), connect: vi.fn() } as never)
    const authority = {
      id: "memory-a" as never,
      group_id: "allura-test" as never,
      workspace_id: "workspace-a",
    ...
    if (method === "softDeleteMemory")
      await adapter.softDeleteMemory({ ...authority, deleted_at: new Date(0).toISOString() })
    else await adapter.restoreMemory({ ...authority, restored_at: new Date(0).toISOString() })
    const sql = client.query.mock.calls.map(([statement]) => String(statement)).join("\n")
    expect(withWorkspaceTransaction).toHaveBeenCalledWith(
      { tenantId: "allura-test", workspaceId: "workspace-a", principalId: "agent-a" },
      expect.any(Function)
  ...
  it("reads deprecated memories only inside the exact workspace", async () => {
    const client = { query: vi.fn(async () => ({ rows: [], rowCount: 0 })) }
    transactionState.client = client
    const adapter = new RuVectorGraphAdapter({ query: vi.fn() } as never)
    await adapter.getDeprecatedMemories({
      ids: ["memory-a"],
      group_id: "allura-test" as never,
      workspace_id: "workspace-a",
      principal_id: "agent-a",
    ...
    expect(client.query).toHaveBeenCalledWith(
      expect.stringMatching(/workspace_id = \$2[\s\S]*workspace_scope_state = 'workspace_scoped'/),
      ["allura-test", "workspace-a", ["memory-a"]]
    )
  })
  ...
  it.each([
    [
      "checkDuplicate",
      (adapter: RuVectorGraphAdapter) =>
        adapter.checkDuplicate({ group_id: "allura-test" as never, user_id: null, content: "x" }),
    ],
    [
      "countMemories",
      (adapter: RuVectorGraphAdapter) => adapter.countMemories({ group_id: "allura-test" as never, user_id: null }),
    ],
    [
      "checkCanonical",
      (adapter: RuVectorGraphAdapter) =>
        adapter.checkCanonical({ id: "memory-a" as never, group_id: "allura-test" as never }),
    ],
    [
      "getVersion",
      (adapter: RuVectorGraphAdapter) =>
        adapter.getVersion({ id: "memory-a" as never, group_id: "allura-test" as never }),
    ],
    [
      "exportMemories",
      (adapter: RuVectorGraphAdapter) =>
        adapter.exportMemories({ group_id: "allura-test" as never, user_id: null, offset: 0, limit: 10 }),
    ],
    [
      "linkMemoryContext",
      (adapter: RuVectorGraphAdapter) =>
        adapter.linkMemoryContext({
          memory_id: "memory-a" as never,
          group_id: "allura-test" as never,
          agent_id: "agent-a",
          project_id: null,
        }),
    ...
    [
      ...
      (adapter: RuVectorGraphAdapter) =>
        adapter.restoreMemory({
          id: "memory-a" as never,
          group_id: "allura-test" as never,
          restored_at: new Date(0).toISOString(),
        }),
    ],
  ] as const)("fails closed without workspace authority for %s", async (_operation, invoke) => {
    const pool = { query: vi.fn(), connect: vi.fn() }
    const adapter = new RuVectorGraphAdapter(pool as never)
    await expect(invoke(adapter)).rejects.toThrow("verified workspace_id and principal_id are required")
    expect(pool.query).not.toHaveBeenCalled()
    expect(pool.connect).not.toHaveBeenCalled()
  })
})
