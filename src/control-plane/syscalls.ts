...
/** Policy inputs a caller may supply for a governed project operation. */
export interface SyscallPolicyContext {
...
export interface SyscallContext {
  ...
  /**
   * Declared governance context for this operation. These values are passed to
   * the real policy evaluator; absence remains a policy failure for writes.
   */
  policy_context?: SyscallPolicyContext;

  /**
   * Opaque, HMAC-verified Genesis evidence. When present, its actor, approval,
   * manifest, and source-read claims replace any caller-provided equivalents.
   */
  genesis_policy_evidence?: string;
...
async function executeSyscall<T>(
  ...
  try {
    // Genesis writes may carry only opaque control-plane evidence. The resolver
    // verifies its HMAC and binds it to this tenant before policy evaluation;
    // raw caller policy_context cannot override verified claims.
    const trustedGenesisEvidence = context.genesis_policy_evidence
      ? resolveGenesisPolicyEvidence(context.genesis_policy_evidence, context.group_id)
      : undefined;
    const effectiveContext: SyscallContext = trustedGenesisEvidence
      ? {
          ...context,
          actor: trustedGenesisEvidence.actor,
          approval_ref: trustedGenesisEvidence.approvalRef,
          policy_context: trustedGenesisEvidence.policyContext,
        }
      : context;
    ...
    const proof = createProof(intent, subject, effectiveContext.actor, {
      group_id: effectiveContext.group_id,
      nonce: undefined as any, // Will be auto-generated by createProof
      budget_cost: effectiveContext.budget_cost,
      permission_tier: effectiveContext.permission_tier,
      audit_context: effectiveContext.audit_context,
    }, secretKey);
    ...
    const policyContext: PolicyContext = {
      ...effectiveContext.policy_context,
      timestamp: Date.now(),
      operation: intent,
      resource: subject,
      actor: effectiveContext.actor,
      budgetLimit: 1000,
      requiredTier: "skill",
      requiresAudit: true,
    };
    ...
    const resolvedApprovalRef = effectiveContext.approval_ref;
